...
def parse_args():
    parser = argparse.ArgumentParser(description='Test DHRNet')
    # general
    parser.add_argument('--cfg',
                        help='experiment configure file name',
                        default='experiments/ochuman_val.yaml',
                        # required=True,
                        type=str)
...
def worker(gpu_id, dataset, indices, cfg, logger, final_output_dir, pred_queue, gpu_list, debug):
    os.environ['CUDA_VISIBLE_DEVICES'] = gpu_list[gpu_id]
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    input_shapes = [{'image': torch.randn(3, 512, 896)}]
    input2_shape = (1,)
    
    model = models.create(cfg.MODEL.NAME, cfg, is_train=False)
    # flops, paras = profile(model.to(device), inputs=input_shapes+[input2_shape])
    ...
    print('Inferencing Speed: ', total_time / 2231)
    # close tqdm
    data_loader.close()
    ...
    for i in range(num_gpus):
        ...
        if not args.debug:
            p = Process(
                target=worker,
                args=(
                    i, test_dataset, indices, cfg, logger, final_output_dir, pred_queue, gpu_list, args.debug
                )
            )
            p.start()
        else:
            worker(i, test_dataset, indices, cfg, logger, final_output_dir, pred_queue, gpu_list, args.debug)
