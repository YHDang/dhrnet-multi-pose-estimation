...
class JointRelationModule(nn.Module):
    ...
    def forward(self, kpt_feat, imgid, j, type=None):
        ...
        imgid = imgid.cpu().numpy().tolist()
        dic, kpt_feats = {}, None
        # print(kpt_feat.shape, len(imgid))
...
class DualRelationModule(nn.Module):
    ...
    def forward(self, instance_feats, kpt_feats, imgid, instance_param, i):
        ...
        # np.save('/data2_12t/user/dyh/results/DHRNet/IJR_feat/IJR_inst_feat_{}.npy'.format(i), inst_relation_feat.cpu().detach().numpy())
        ...
        # np.save('/data2_12t/user/dyh/results/DHRNet/IJR_feat/IJR_joint_feat_{}.npy'.format(i), kpt_relation_feat_1.cpu().detach().numpy())


        # Joint-Instance order
        kpt_relation_feat = self.kpts_relation_2(kpt_feats, imgid, i, type='ji')
        # np.save('/data2_12t/user/dyh/results/DHRNet/JIR_feat/JIR_joint_feat_{}.npy'.format(i), kpt_relation_feat.cpu().detach().numpy())
        ...
        # np.save('/data2_12t/user/dyh/results/DHRNet/JIR_feat/JIR_inst_feat_{}.npy'.format(i), inst_relation_feat_2.cpu().detach().numpy())
